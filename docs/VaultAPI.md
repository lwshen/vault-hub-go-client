...
[**GetVaultFilterOptions**](VaultAPI.md#GetVaultFilterOptions) | **Get** /api/vaults/filter-options | 
...
> VaultFilterOptionsResponse GetVaultFilterOptions(ctx).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.VaultAPI.GetVaultFilterOptions(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `VaultAPI.GetVaultFilterOptions``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetVaultFilterOptions`: VaultFilterOptionsResponse
	fmt.Fprintf(os.Stdout, "Response from `VaultAPI.GetVaultFilterOptions`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetVaultFilterOptionsRequest struct via the builder pattern


### Return type

[**VaultFilterOptionsResponse**](VaultFilterOptionsResponse.md)
